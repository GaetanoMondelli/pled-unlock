...
export async function matchEventToRule(event: any, rule: any, variables: any = {}): Promise<boolean> {
...
export async function getMatchingRules(event: any, rules: any[], variables: any = {}): Promise<any[]> {
  // Sort rules by priority (highest first)
  const sortedRules = [...rules].sort((a, b) => getRulePriority(b) - getRulePriority(a));
  
  if (LAZY_RULE_MATCHING) {
    // Lazy evaluation - return first matching rule
    for (const rule of sortedRules) {
      const matches = await matchEventToRule(event, rule, variables);
      if (matches) {
        return [rule];
      }
    }
    ...
    const results = await Promise.all(
      sortedRules.map(async rule => ({
        rule,
        matches: await matchEventToRule(event, rule, variables)
      }))
    );
