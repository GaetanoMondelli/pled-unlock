...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/accounts/${accountId}/envelopes/${params.id}`, {
      headers: {
    ...
    return NextResponse.json({ error: error.message || "Failed to get envelope status" }, { status: 500 });
  }
}
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const getResponse = await fetch(`${BASE_URL}/accounts/${accountId}/envelopes/${params.id}`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
    ...
    if (data.recipients?.length > 0) {
      ...
      await fetch(`${BASE_URL}/accounts/${accountId}/envelopes/${params.id}/recipients`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      ...
      // Add new recipients
      const recipientsResponse = await fetch(`${BASE_URL}/accounts/${accountId}/envelopes/${params.id}/recipients`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
    ...
    const updateResponse = await fetch(`${BASE_URL}/accounts/${accountId}/envelopes/${params.id}`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
    ...
    // Get final status
    const finalResponse = await fetch(`${BASE_URL}/accounts/${accountId}/envelopes/${params.id}`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
    ...
    return NextResponse.json({ error: error.message || "Failed to send envelope" }, { status: 500 });
  }
}
