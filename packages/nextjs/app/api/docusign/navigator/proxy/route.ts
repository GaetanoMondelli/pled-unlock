...
export async function POST(req: Request) {
  try {
    ...
    if (responseText.includes("<!DOCTYPE html>")) {
      ...
      return NextResponse.json({ error: "Authentication failed - token not valid for Navigator API" }, { status: 401 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Invalid response from Navigator API" }, { status: response.status });
    }
  } catch (error: any) {
    console.error("[Navigator Proxy] Error:", error);
    return NextResponse.json({ error: error.message || "Navigator proxy request failed" }, { status: 500 });
  }
}
