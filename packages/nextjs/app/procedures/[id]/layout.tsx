...
export default function ProcedureLayout({ children, params }: { children: React.ReactNode; params: { id: string } }) {
    ...
    async function loadData() {
      try {
        ...
        const template = instance
          ? dbData.procedureTemplates?.find((t: any) => t.templateId === instance.templateId)
          : null;
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "variables":
        return <VariablesSection procedureId={params.id} template={template} instance={instance} />;
      case "events":
        return (
          <Card className="p-4">
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 p-4 text-sm border-b">
        ...
        <span>
          {template.name} - {instance.variables?.candidate?.name}
        </span>
      </div>
