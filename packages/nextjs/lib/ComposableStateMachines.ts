...
export class MessageBus {
  ...
  handleStateEmission(componentId: string, stateId: string, messageType: string, data: any) {
    const matchingRoutes = this.routes.filter(
      r => r.from.componentId === componentId && r.from.stateId === stateId && r.from.messageType === messageType,
    );
...
export class ComponentComposer {
  static compileComponents(components: ComposableComponent[], routes: MessageRoute[]): CompiledWorkflow {
