...
import { Play, Pause, Square, RotateCcw, Activity, Clock, Zap, CircuitBoard } from 'lucide-react';
  ...
  return (
    <Card className={`w-52 shadow-lg ${data.isActive ? 'animate-pulse border-cyan-500' : ''}`}>
      ...
      <CardContent className="p-3 text-xs">
        ...
        <p>Range: [{config.valueMin || 1}-{config.valueMax || 100}]</p>
  ...
  return (
    <Card className={`w-60 shadow-lg ${data.isActive ? 'animate-pulse border-violet-500' : ''}`}>
      ...
      <CardContent className="p-3 text-xs">
        ...
        <p>Formula: {(config.formulas?.[0] || 'input1.value').substring(0, 20)}{(config.formulas?.[0]?.length || 0) > 20 ? '...' : ''}</p>
        {data.details && <p className="mt-1 text-gray-600">{data.details}</p>}
        {data.error && <p className="mt-1 text-red-600">{data.error}</p>}
      </CardContent>
      ...
      setCurrentTime(prev => {
        ...
        scenarioNodes.forEach(node => {
          if (node.data.type === 'DataSource' && newTime % (node.data.config.interval || 3) === 0) {
            const value = Math.floor(Math.random() * ((node.data.config.valueMax || 10) - (node.data.config.valueMin || 1) + 1)) + (node.data.config.valueMin || 1);
            ...
            setScenarioNodes(prev => prev.map(n => 
              n.id === node.id 
                ? { ...n, data: { ...n.data, isActive: true, details: `Generated: ${value}` } }
                : n
            ));
            
            // Animate connected edges
            setScenarioEdges(prev => prev.map(edge => 
              edge.source === node.id 
            ...
            setTimeout(() => {
              setScenarioNodes(prev => prev.map(n => 
                n.id === node.id 
                  ? { ...n, data: { ...n.data, isActive: false, details: '' } }
                  : n
              ));
              setScenarioEdges(prev => prev.map(edge => 
                edge.source === node.id 
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-500">
        <p>🎯 <strong>Working Simulation:</strong> Data Source generates tokens every 3 seconds → Queue aggregates them → Sink consumes them</p>
        <p>💡 <strong>How to extend:</strong> You can drag nodes around, connect them differently, and they'll work like the tokenization demo</p>
      </div>
    </div>
  );
};
