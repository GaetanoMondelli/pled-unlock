                  ...
                  <tbody className="divide-y divide-gray-100">
                    {event.template.history.events.map((historyEvent: any) => (
                      <tr key={historyEvent.id} className="hover:bg-gray-50">
                        <td className="px-2 py-1 text-xs">{new Date(historyEvent.timestamp).toLocaleString()}</td>
                        <td className="px-2 py-1 text-xs">{historyEvent.type}</td>
                        <td className="px-2 py-1 text-xs">{JSON.stringify(historyEvent.data)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
